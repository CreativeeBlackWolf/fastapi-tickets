...
async def list_tickets(limit: int = 27, db: Session = Depends(get_db)):
...
async def get_single_ticket(pk: int, db: Session = Depends(get_db)):
...
async def update_ticket(pk: int, ticket: TicketUpdate, db: Session = Depends(get_db)):
    """
